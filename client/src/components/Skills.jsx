    ...
    <section id="skills" className="py-24 relative">
      ...
      >
        <div className="flex items-center gap-4 mb-12">
          <h2 className="text-4xl font-bold text-white"><span className="text-emerald-500">02.</span> My Skills</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillsData.map((skillGroup, index) => (
            ...
            >
              <h3 className="text-xl font-orbitron font-bold text-white mb-6 text-center">{skillGroup.category}</h3>
              <div className="flex flex-wrap gap-3 justify-center">
                {skillGroup.items.map((skill, idx) => (
                  <span key={idx} className="px-3 py-1 bg-slate-800/80 text-emerald-400 rounded-full text-sm font-medium shadow-sm border border-emerald-500/20">
                    {skill}
                  </span>
                ))}
              </div>
